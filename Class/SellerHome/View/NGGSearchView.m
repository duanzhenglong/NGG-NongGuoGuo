...
@interface NGGSearchView()<UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate>{
...
@implementation NGGSearchView

static NGGSearchView *SearchView;
/**
 *显示搜索界面
 */
+(void)ShowSeatchView{
    if (!SearchView) {
        ...
        SearchView=[[NGGSearchView alloc]initWithFrame:CGRectMake(0, 0, window.ngg_width, window.ngg_height)];
        SearchView.alpha=0;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    [manager POST:GoodsIdNeedlistURL parameters:myParameters success:^(NSURLSessionDataTask *task, id responseObject) {
