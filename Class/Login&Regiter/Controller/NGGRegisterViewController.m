...
-(void)layoutLoginView{
    ...
    /*创建验证码label*/
    UILabel*VerifiLabel=[[UILabel alloc]initWithFrame:CGRectMake(15, passwordLabel.ngg_bottom+40, 70, 25)];
    ...
    /*创建验证码Text*/
    VerificationText=[[NGGEditTextField alloc]initWithFrame:CGRectMake(VerifiLabel.ngg_right+10, VerifiLabel.ngg_y, SCREEN_WIDTH-VerifiLabel.ngg_width*2, VerifiLabel.ngg_height)];
...
-(void)RegisterUser{
    ...
    [manager POST:RegisterURL parameters:myParameters success:^(NSURLSessionDataTask *task, id responseObject) {
    
        NSString*str=responseObject[@"message"];
        if ([str isEqualToString:@"注册成功！"]) {
            NSDictionary*dic=responseObject[@"result1"];
             /*立即登陆当前用户*/
            ...
            [dic writeToFile:path atomically:YES];
             /*注册到环信服务器*/
//            [self registerHyphenate];
            [self dismissViewControllerAnimated:YES completion:nil];
        }else{
            UIAlertController*alert=[UIAlertController alertControllerWithTitle:@"提示" message:str preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction*other=[UIAlertAction actionWithTitle:@"换一个试试吧！" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
